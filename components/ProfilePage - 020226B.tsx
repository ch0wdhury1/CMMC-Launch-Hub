...
<div className="rounded-lg border bg-slate-50 p-4">
      ...
      {JSON.stringify(
        {
          ...
          meta: {
            activeMemberCount: orgDoc?.activeMemberCount ?? null,
            maxUsers: orgDoc?.maxUsers ?? null,
            billingCycle: orgDoc?.billingCycle ?? null,
          },
  ...
  {!orgDocLoading && orgDoc && tier && orgDoc?.tier && String(tier) !== String(orgDoc.tier) && (
    <div className="mt-2 text-amber-700 text-xs">
      Note: user tier ({String(tier)}) does not match org tier ({String(orgDoc.tier)}). Org doc is the source of truth.
    </div>
  )}
</div>
