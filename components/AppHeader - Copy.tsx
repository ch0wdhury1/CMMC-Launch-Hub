...
interface AppHeaderProps {
  ...
  onProfileClick: () => void;
  onDiagnosticsClick?: () => void; // New prop
  overallCompletion: number;
  sprsScore: number;

  //  NEW: optional logout handler
  onLogout?: () => void;
}
...
export const AppHeader: React.FC<AppHeaderProps> = ({
  ...
  const isDev = process.env.NODE_ENV === 'development' || true; // Flag true for preview/visibility in this context
  ...
  return (
    <header className="w-full bg-blue-800 border-b border-blue-900 h-20 flex items-center px-6 justify-between shadow-md z-50 flex-shrink-0">
      ...
      <div className="flex items-center">
        <div className="flex flex-col">
          ...
          <span className="text-base text-blue-300">
            Basic Safeguarding Starts Here...
          </span>
        </div>
      </div>

      {/* CENTER: PROGRESS BAR + SPRS SCORE */}
      ...
      <div className="flex items-center space-x-2">
        ...
        {/*  NEW: Logout button (only shows if onLogout is passed) */}
        {onLogout && (
          <button
            onClick={onLogout}
