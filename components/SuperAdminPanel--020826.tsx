...
export const SuperAdminPanel: React.FC = () => {
  ...
  const approveRegistration = async (req: AccessRequestRow) => {
    ...
    // ✅ Stable orgId prevents duplicate orgs on double-click
    const stableOrgId = safeStr((req as any).orgId) || `org_${req.id}`;
    ...
    try {
      ...
      const baseOrgId = `org_${slugify(orgName) || "new"}`;
      let orgId = baseOrgId;
      // ensure unique
      const exists = await getDoc(doc(db, "orgs", orgId));
      if (exists.exists()) {
        orgId = `${baseOrgId}_${Math.random().toString(36).slice(2, 6)}`;
      }
      ...
      const isSponsored = requestedTier === "SPONSORED" || requestedTier === "CT_SPONSORED";
      const maxUsers = isSponsored ? 1 : 10;

      // ORG doc (source of truth)
      const orgDocData = stripUndefined({
        name: orgName,
        address: safeStr((req as any).address).trim(),
        website: safeStr((req as any).website).trim(),
        tier: requestedTier,
        ...
        maxUsers,
        activeMemberCount: 1,
        primaryContactName: safeStr((req as any).primaryContactName).trim(),
        primaryContactEmail:
          safeStr((req as any).primaryContactEmail || (req as any).email || (req as any).ownerEmail)
            .trim()
            .toLowerCase(),
        primaryContactPhone: safeStr((req as any).primaryContactPhone).trim(),
      ...
      await setDoc(doc(db, "orgs", orgId), orgDocData, { merge: true });

      // org member record
      await setDoc(
        doc(db, "orgs", orgId, "members", ownerUid),
        {
          createdAt: serverTimestamp(),
          role: "orgAdmin",
          superAdmin: false,
      ...
      // user doc: attach orgId + activate
      const userUpdate = stripUndefined({
        ...
        email:
          safeStr((req as any).primaryContactEmail || (req as any).email || (req as any).ownerEmail)
            .trim()
            .toLowerCase(),
        fullName: safeStr((req as any).primaryContactName || (req as any).fullName).trim(),
        phone: safeStr((req as any).primaryContactPhone).trim(),
        status: "active",
        tier: requestedTier,
        track: safeStr((req as any).track || "TRACK_1"),
      ...
      await setDoc(doc(db, "users", ownerUid), userUpdate, { merge: true });
...
// ✅ remove from local pending list immediately (prevents double-approve)
